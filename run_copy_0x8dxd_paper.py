"""
PAPER TRADING copy trader for 0x8dxd
...
SCALE = 0.10
POLL_SEC = 10
STATE_FILE = 'paper_0x8dxd_state.json'

seen = set()
paper_trades = []
paper_balance = 1000.0  # Start with $1000 paper
...
def save_state():
    with open(STATE_FILE, 'w') as f:
        json.dump({
            'seen': list(seen)[-1000:],  # Keep last 1000
            'paper_trades': paper_trades[-100:],  # Keep last 100
...
def get_report():
    ...
    win_rate = wins / max(1, len(closed)) * 100
    roi = (paper_balance - starting_balance) / starting_balance * 100

    return f"""
{'='*50}
0x8dxd PAPER TRADING REPORT
{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
{'='*50}
Starting Balance: ${starting_balance:.2f}
Current Balance:  ${paper_balance:.2f}
ROI:              {roi:+.2f}%
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        while True:
            try:
                ...
                # Check for resolved markets (update open trades)
